...
Uso:
  python backtest.py                                    # todos os presets, BTC+ETH, 180 dias
  python backtest.py --pairs BTC/USDT,ETH/USDT          # pares específicos
  python backtest.py --strategy elder_balanceado        # uma estratégia
...
def build_daily_ema_lookup(
    daily_candles: list, period: int
) -> dict[int, float | None]:
...
def get_daily_ema_at(ts_ms: int, daily_ema_lookup: dict) -> float | None:
...
class _Pos:
    ...
    def __init__(self):
        ...
        self.avg_price          = 0.0
        self.total_qty          = 0.0
        self.total_invested     = 0.0   # USDT sem taxas
...
def simulate(
    ...
    pos            = _Pos()
    trades         = []
    equity_curve   = []   # [(datetime, cumulative_pnl)]
    entries        = []   # [(datetime, price, order_num)]
    exits          = []   # [(datetime, price, reason, pnl_net)]
    cumulative_pnl = 0.0
    max_drawdown   = 0.0
    ...
    for i, candle in enumerate(hourly_candles):
        ...
        # ── Sem posição — verifica entrada ────────────────────────────────────
            ...
            # Circuit Breaker: para backtest single-pair, usa drawdown atual do portfólio
            # Sem posição aberta, CB não bloqueia (não há drawdown)
...
def plot_pair(pair: str, results: list[dict], hourly_candles: list, output_dir: Path) -> None:
...
def plot_summary_bar(all_results: dict[str, list[dict]], output_dir: Path) -> None:
...
def main():
    ...
    parser.add_argument(
        "--pairs", default="BTC/USDT,ETH/USDT",
        help="Pares separados por vírgula (padrão: BTC/USDT,ETH/USDT)",
    )
    parser.add_argument(
    ...
    for pair in pairs:
        ...
        # Busca dados (1h para simulação, 1d para EMA)
        hourly = fetch_history(ex, pair, "1h", args.days + 2)
        if not hourly:
            print(f"  ⚠️  Sem dados para {pair}. Pulando.")
            continue

        # Dados diários para EMA (precisa de period+10 dias extras)
        max_ema = max(
        ...
        daily_closes = [float(c[4]) for c in daily]

        # RSI série horária (calculada uma vez, reutilizada por todas as estratégias)
        ...
        for strat_key in strategies:
            ...
            # EMA lookup diário (só se filtro ativo)
